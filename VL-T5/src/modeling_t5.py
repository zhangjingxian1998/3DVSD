...
class VisualEmbedding(nn.Module):
    ...
    def forward(self, feats, pos, img_order_ids=None, obj_order_ids=None):
        ...
        feat_embedding = self.feat_embedding(feats) # 先降维

        device = feats.device
        dtype = feats.dtype

        area = self.get_area(pos).unsqueeze(2) # [B, N, 1] # 框的包围面积
        pos = torch.cat([pos, area], dim=2) # [B, N, 5]

        # [B, N, d_model]
        absolute_vis_pos_embedding = self.absolute_vis_pos_embedding(pos) # (5) -> (768)
        ...
        if self.config.use_vis_order_embedding:
            if img_order_ids is None:
                ...
                img_order_ids = img_order_ids.unsqueeze(0) #.expand(B, -1)      # 只有一张图片id都是0
            ...
            if obj_order_ids is None:
                ...
                obj_order_ids = obj_order_ids.unsqueeze(0) #.expand(B,-1)       # 顺序排列
            # assert obj_order_ids.max().item() < 32200, obj_order_ids
            obj_order_ids = self.obj_order_embedding.num_embeddings - obj_order_ids - 1 # 这一步是为什么
            obj_order_embedding = self.obj_order_embedding(obj_order_ids)

            vis_embedding = feat_embedding + absolute_vis_pos_embedding + \
                img_order_embedding + obj_order_embedding # 这里就是图2的b
...
class JointEncoder(T5Stack):
    ...
    def forward(
        ...
        img_order_ids = None
        obj_order_ids = None
        if len(vis_inputs) >= 3: # 有多张图片输入的情况，会有一个排序的
            img_order_ids = vis_inputs[2]
        if len(vis_inputs) == 4:
            obj_order_ids = vis_inputs[3]

        vis_embeds = self.visual_embedding(
            vis_feats, boxes, img_order_ids, obj_order_ids) # (1,36,2048) (1,36,4)

        V_L = vis_embeds.size(1)

        inputs_embeds = torch.cat([inputs_embeds, vis_embeds], dim=1) # 文本和视觉拼接
        ...
        attention_mask = torch.cat([attention_mask, vis_attention_mask], dim=1) # 对应上面的文本视觉拼接
...
class VLT5(T5ForConditionalGeneration):
    ...
    def forward(
        ...
        **kwargs,
        ...
        lm_logits = self.lm_head(sequence_output) # 768 --> 32200
