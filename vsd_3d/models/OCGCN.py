...
class OcGCN(nn.Module):
    ...
    '''
        ...
        self.vison = FFN(in_feature = 2048, 
                         middle_feature = 1024, 
                         out_feature = 768)
        self.edge = FFN(in_feature=1,middle_feature=1024,out_feature=768)

        self.Wb = nn.ModuleList()
        for _ in range(layer_num):
            self.Wb.append(nn.Linear(768,768)) # 这里是输出1还是输出768
    ...
    def forward(self,x, vis_feats, adjacency_matrix):
        ...
        for idx_layer in range(len(self.Wa)): # 公式2
            ...
            a = torch.exp(a)
            a = a * adjacency_matrix_mask_float             # 只对存在关系的连边保持梯度
            ...
            b = torch.exp(b)
            b = b * adjacency_matrix_mask_float             # 只对存在关系的连边保持梯度
            ...
            vision = vision.unsqueeze(2).repeat(1,1,N,1)                # [B, N, D] --> [B, N, 1, D] --> [B, N, N, D]
    ...
    def preprocess(self, data, vision, adjacency_matrix):
        ...
        s_e_res = self.edge(s_e.view(B, N*N, 1))                                # 每个batch展平送入网络处理 [B, N, N] --> [B, N*N, 1] --> [B, N*N, D]
        s_e_result = s_e_res.view(B, N, N, -1) * adjacency_matrix_mask_float    # 网络处理完恢复尺寸 [B, N*N, D] --> [B, N, N, D] 并仅使存在关系的边保持梯度
